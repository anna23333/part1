...
int ABT_init(int argc, char **argv)
{
    ...
    gp_ABTI_global->empty_list = (int *)malloc(80*sizeof(int));
    gp_ABTI_global->state_list = (char *)malloc(40*sizeof(char));

    gp_ABTI_global->es_state = (char *)malloc(40*sizeof(char));
    gp_ABTI_global->pool_state = (char *)malloc(40*sizeof(char));
    
    for(int i = 0; i < 40; i++) {
        gp_ABTI_global->state_list[i] = '0';
        gp_ABTI_global->es_state[i] = '0';
        gp_ABTI_global->pool_state[i] = '0';
    }
    for(int i = 0; i < 80; i++)
    {
        gp_ABTI_global->empty_list[i] = 0;
        gp_ABTI_global->empty_set[i] = 0;
        gp_ABTI_global->check_set[i] = 0;
        gp_ABTI_global->states[(i+1)/2] = 0;
    }
    gp_ABTI_global->state_list[80] = '\0';

//    for(int j = 0; j < 40; j++)
//        gp_ABTI_global->state_list[j]='2';
//    gp_ABTI_global->state_list[40]='2';

    gp_ABTI_global->threshold = 2;        //the threshold of the pool, not sure of this num, depends on the benchmark of the overhead.
    gp_ABTI_global->ll = 0;

  fn_exit:
    printf("global.init: threshold = 2\n");
    return abt_errno;
