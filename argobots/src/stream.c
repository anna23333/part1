...
void ABT_info(ABT_pool *pools, int check_num)
{
    ABT_pool pool0 = pools[0];
    ABT_pool pool1 = pools[1];


    ABTI_pool *p_pool = ABTI_pool_get_ptr(pool0);
    uint64_t pool_id = p_pool->id;

    for(int i = 1; i < 41; i++)
    {
        ABT_pool i_pool = pools[i];
        ABTI_pool *p_i_pool = ABTI_pool_get_ptr(i_pool);
        uint64_t i_pool_id = p_i_pool->id;
        size_t i_size;
        ABT_pool_get_size(p_i_pool, &i_size);
//        printf("id%lu\t--size:%lu\n", i_pool_id, i_size);

    }
}


void ABT_print_my_info(ABT_pool *pools)
{
    ABTI_pool *p_0 = ABTI_pool_get_ptr(pools[0]);
    ABTI_pool *p_1 = ABTI_pool_get_ptr(pools[1]);
    ...
    for(int i = 0; i < 41; i++)
    {
        ABTI_pool *p = ABTI_pool_get_ptr(pools[i]);
        ABT_pool_get_size(pools[i], &size);
        printf("i=%d --> pool%lu --> size = %lu\n", i, p->id, size);
    }



    ABT_pool_get_size(pools[0], &size0);
    ABT_pool_get_size(pools[1], &size1);

    printf("pool0=%lu,size0=%lu; pool1=%lu,size=%lu\n", p_0->id, size0, p_1->id, size1);

}

void ABT_print_my_sched(ABT_sched sched)
{
...
//    if(p_sched->id == 40)
    {
        ...
        for(int j = 0; j < 41; j++) 
        {
            int id = (int)p_sched->id;
            if(j == 0)
                printf("%d ", 2*id-1);
            else if(2*(id+j-1) <= 80)
                printf("%d ", 2*(id+j-1));
            else
                printf("%d ", 2*(id+j-1)-80);
        }
        printf("\n");
#endif
    }
    ...
    char flag = gp_ABTI_global->state_list[28];
    printf("%c\n", flag);
    if(flag == '9')
        printf("yes\n");
...
void ABT_check_pool(ABT_pool *pools, int check_num)
{
    ABT_pool pool0 = pools[0];
    ABT_pool pool1 = pools[1];
...
void ABT_write_to_pipe(int fd_fifo)
{
#if 0
    int ret = write(fd_fifo, gp_ABTI_global->state_list, strlen(gp_ABTI_global->state_list)+1);
    if(ret < 0){
...
void ABT_pipe_set_info(ABT_sched sched)
{
    ABTI_sched *p_sched = ABTI_sched_get_ptr(sched);

    int num;
    ABT_sched_get_num_pools(sched, &num);
    ABT_pool *pools;
    pools = (ABT_pool *)malloc(num * sizeof(ABT_pool));
    int ret = ABT_sched_get_pools(sched, num, 0, pools);

    int index;
    index = rand() % (41-2) ;

    ABTI_pool *p_pool = ABTI_pool_get_ptr(pools[index]);
    int list_id = p_pool->id;

    size_t size;
    ABT_pool_get_size(pools[index], &size);
    if(size > 0)
        gp_ABTI_global->state_list[list_id] = '1';
    else
        gp_ABTI_global->state_list[list_id] = '0';
}


int ABT_xstream_check(ABT_pool *pools, ABT_pool pool, int *run_cnt, ABT_sched sched)
{
//    int abt_errno = ABT_SUCCESS;
    ...
    {
        ABT_my_xstream_run_unit(unit, pool, sched);
//        abt_errno = ABT_my_xstream_run_unit(unit, pool, sched);
        run_cnt++;
    }

    if(gp_ABTI_global->state_list[id/2]=='0') {
        size_t size;
...
#if 0
int calculate(int sched_id, int i)
{
    return (sched_id + i) % 80;
}
#endif

    
void ABT_set_r_info(ABT_sched sched)
{
    ABTI_sched *p_sched = ABTI_sched_get_ptr(sched);
    ...
    ABT_pool *pools;
    pools = (ABT_pool *)malloc(num * sizeof(ABT_pool));
    ABT_sched_get_pools(sched, num, 0, pools);

    ABTI_pool *p_pool = ABTI_pool_get_ptr(pools[1]);
    int list_id = p_pool->id;
    size_t size;
    ABT_pool_get_size(pools[1], &size);
    if(size >= gp_ABTI_global->threshold)
        gp_ABTI_global->empty_list[list_id] = 1;
#if 0
    for(int i = 1; i < 41; i++)
    {
        ABTI_pool *p_pool = ABTI_pool_get_ptr(pools[i]);
...
void ABT_set_s_info(ABT_sched sched)
{
    ABTI_sched *p_sched = ABTI_sched_get_ptr(sched);
    ...
    ABT_pool_get_size(pools[1], &size);
    if(size < gp_ABTI_global->threshold)
        gp_ABTI_global->empty_list[list_id] = 0;
...
{
    int i = random()%40;
    if(gp_ABTI_global->es_state[i]=='0' && gp_ABTI_global->pool_state[i]=='0')
        gp_ABTI_global->state_list[i] = '0';
    else
        gp_ABTI_global->state_list[i] = '1';

}


int ABT_change_id_pool(int sched_id, int pool_id)
{
    int return_id;
...
{
    int target, target_second;
    size_t size;

    int threshold = gp_ABTI_global->threshold;
    ...
    if(gp_ABTI_global->empty_list[target] == 1)
    {
...
int ABT_my_migrate2(ABT_pool *pools, ABT_sched sched, int *run_cnt)
{
...
//    printf("\n");
    ...
    if(p_pool->id > 0)
    {//22-id(id<22) + 62-id(else) : pools[target]=42, the first pool to empty.
//#if 0
        if(sched_id < 22)
            target = 22 - sched_id;
        else
...
//#endif
...
#if 0
        if(sched_id < 22)
            target = 27 - sched_id - loop;
        else
            target = 67 - sched_id - loop;
#endif
        ...
        while(size > 0)
        {
            ...
            if(unit != ABT_UNIT_NULL){
                ABT_unit_set_associated_pool(unit, pool);
                ABT_pool_push(pool, unit);
       //         printf("sched_id = %lu, pop from %lu, push to %lu\n ", sched_id, ABTI_pool_get_ptr(pools[target])->id, ABTI_pool_get_ptr(pools[1])->id);
...
void ABT_es_info(ABTI_xstream *p_xstream)
{
    int num = gp_ABTI_global->num_xstreams;
//    for(int i = 0; i < num; i++)
    {
//        ABTI_xstream *p_es = gp_ABTI_global->p_xstreams[i];
        ABT_xstream_state state;
        ABT_xstream es = ABTI_xstream_get_handle(p_xstream);
        ABT_xstream_get_state(es, &state);

        int i = p_xstream->rank;
        if(state == 0)
            gp_ABTI_global->es_state[i] = '0';
        else
            gp_ABTI_global->es_state[i] = '1';
//        printf("%d ", state);
    }
//    printf("\n");
}
...
//        {
//            gp_ABTI_global->ll = 1;
//            ABTI_spinlock_acquire(&p_xstream->state_lock);
//            ABT_es_info();
//            ABTI_spinlock_release(&p_xstream->state_lock);
//            gp_ABTI_global->ll = 0;
//        }
//        freq++;
