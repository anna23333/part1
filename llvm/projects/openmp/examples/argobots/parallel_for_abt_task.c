...
/*  parallel_for_abt_task.c code mimics the parallel for OpenMP directive.  It
 *  creates as many ESs as user requires, and tasks are created and assigned by
 *  static blocks to each ES.
 */
...
int main(int argc, char *argv[])
{
    ...
    for (j = 0; j < num_xstreams; j++) {
        ...
        args[j].value = 0.9f;
        args[j].ptr = a;
        ABT_task_create_on_xstream(xstreams[j], vector_scal,
                                   (void *)&args[j], &tasks[j]);
    }
